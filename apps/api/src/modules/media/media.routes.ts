...
import {
  createTempAsset,
  deleteTempAsset,
} from "./temp-media.store.js";
import {
  supportedCapturePoses,
  validateFaceCaptureFile,
} from "./face-capture-validation.service.js";
...
const upload = multer({
  ...
  fileFilter: (_request, file, callback) => {
    ...
    if (!validMimeTypes.includes(file.mimetype)) {
      callback(
        new AppError(422, "INVALID_FILE_TYPE", "Only JPG and PNG uploads are supported."),
      );
      return;
    }
...
mediaRouter.post(
  ...
  async (request, response, next) => {
    try {
      ...
      const pose = supportedCapturePoses.includes(
        requestedPose as (typeof supportedCapturePoses)[number],
      )
        ? (requestedPose as (typeof supportedCapturePoses)[number])
        : "LOOK_FORWARD";
...
mediaRouter.delete(
  "/api/v1/media/temporary-assets/:assetId",
  async (request, response, next) => {
    try {
