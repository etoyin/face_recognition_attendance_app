...
import {
  calculateBlurScore,
  isPoseSatisfied,
} from "../modules/media/face-capture-validation.service.js";
...
describe("face capture validation helpers", () => {
  it("reports sharper images with a higher blur score", () => {
    ...
    expect(calculateBlurScore(sharpImage)).toBeGreaterThan(
      calculateBlurScore(softImage),
    );
  });

  it("matches smile and neutral poses using facial metrics", () => {
    expect(
      isPoseSatisfied("SMILE", {
        isPositioned: true,
        yaw: 0.01,
        pitch: 0.01,
        smileScore: 0.82,
