...
export async function createTempAsset(input) {
    ...
    const expiresAt = new Date(now.getTime() + env.uploadTtlHours * 60 * 60 * 1000).toISOString();
    const asset = {
        assetId,
...
export async function getTempAsset(assetId) {
...
export async function commitAssets(assetIds, memberId, transaction) {
...
export async function validateCommittedCandidateAssets(input) {
    const profilePhoto = await TemporaryMediaAsset.findByPk(input.profilePhotoAssetId);
    ...
    if (faceCaptures.some((asset) => asset.assetType !== "FACE_CAPTURE" || asset.uploadStatus === "EXPIRED")) {
    ...
    if (profilePhoto.uploadStatus === "EXPIRED") {
        throw new AppError(422, "INVALID_PROFILE_PHOTO", "Profile photo asset has expired.");
    }
