...
let serverFaceValidationAvailable = typeof globalThis.document !== "undefined";
...
function analyzeFace(landmarks, blendshapeCategories = []) {
    ...
    const isPositioned = faceWidth >= 0.22 &&
        faceHeight >= 0.28 &&
        bounds.minX >= 0.08 &&
        bounds.maxX <= 0.92 &&
        bounds.minY >= 0.08 &&
...
export function isPoseSatisfied(pose, analysis) {
...
export function calculateBlurScore(imageData) {
    ...
    for (let y = 1; y < height - 1; y += 1) {
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const laplacian = grayscale[index - width] +
                grayscale[index - 1] +
                grayscale[index + 1] +
...
async function getLandmarker() {
    if (!serverFaceValidationAvailable) {
        throw new AppError(503, "FACE_VALIDATION_UNAVAILABLE", "Face validation is temporarily unavailable. Try again in a moment.");
    }
    if (!landmarkerPromise) {
        landmarkerPromise = (async () => {
            const wasmRoot = resolve(process.cwd(), "..", "..", "node_modules", "@mediapipe", "tasks-vision", "wasm");
            const visionFileset = await FilesetResolver.forVisionTasks(pathToFileURL(wasmRoot).href);
...
export async function validateFaceCaptureFile(input) {
    ...
    try {
        ...
        throw new AppError(422, "INVALID_IMAGE_FILE", "The uploaded face capture could not be processed as a valid image.");
    }
    if (originalWidth < minImageDimension ||
        originalHeight < minImageDimension) {
        throw new AppError(422, "FACE_CAPTURE_TOO_SMALL", "The captured image is too small. Move closer and try again.", {
            image: [
    ...
    if (blurScore < minBlurVariance) {
        throw new AppError(422, "FACE_CAPTURE_BLURRY", "The captured image is blurry. Hold still and try again.", {
    ...
    try {
        ...
        const detected = landmarker.detect(imageData);
        detectionResult = {
            faceLandmarks: detected.faceLandmarks,
            faceBlendshapes: detected.faceBlendshapes,
        };
    }
    catch (error) {
        if (error instanceof Error &&
    ...
    if (faceCount === 0) {
        throw new AppError(422, "FACE_NOT_DETECTED", "No face was detected in the captured image. Position one person in the frame and try again.");
    }
    if (faceCount > 1) {
        throw new AppError(422, "MULTIPLE_FACES_DETECTED", "Multiple faces were detected. Ensure only one person is visible before capturing.");
    }
    const analysis = analyzeFace(detectionResult.faceLandmarks[0], detectionResult.faceBlendshapes?.[0]?.categories ?? []);
    if (!analysis.isPositioned) {
        throw new AppError(422, "FACE_NOT_CENTERED", "One face was found, but it is too close to the edge or too small in the frame.", {
            image: ["Center the face and move slightly closer before capturing."],
        });
    }
    if (!isPoseSatisfied(input.requestedPose, analysis)) {
        throw new AppError(422, "WRONG_FACE_POSE", `The captured image does not match the required ${input.requestedPose.replace(/_/g, " ").toLowerCase()} pose.`, {
