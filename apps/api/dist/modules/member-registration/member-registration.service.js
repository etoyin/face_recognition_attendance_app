...
import { commitAssets, validateCommittedCandidateAssets, } from "../media/temp-media.store.js";
export function generateMembershipId(count, year) {
    return `MEM-${year}-${String(count + 1).padStart(4, "0")}`;
}
async function getNextMembershipId(year, transaction) {
...
export async function createMemberRegistration(payload) {
    if (payload.personalInformation.email &&
        (await Member.count({
            where: {
    ...
    try {
        const result = await sequelize.transaction(async (transaction) => {
            const membershipId = await getNextMembershipId(new Date().getFullYear(), transaction);
            await Member.create({
                memberId,
                membershipId,
                ...
                weddingAnniversary: payload.personalInformation.weddingAnniversary || null,
                ...
                emergencyContactRelationship: payload.personalInformation.emergencyContactRelationship,
        ...
        if (error instanceof UniqueConstraintError) {
            throw new AppError(409, "UNIQUE_CONSTRAINT_ERROR", "Membership or email already exists.");
        }
        throw error;
    }
}
