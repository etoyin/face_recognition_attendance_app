...
memberRegistrationRouter.post("/api/v1/members/registration", async (request, response, next) => {
    try {
        const parsed = memberRegistrationRequestSchema.safeParse(request.body);
        if (!parsed.success) {
            throw new AppError(400, "VALIDATION_ERROR", "The registration payload is invalid.", parsed.error.flatten().fieldErrors);
        }
