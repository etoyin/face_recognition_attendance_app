...
export async function runMigrations(sequelize) {
  ...
  `);
    for (const migration of migrations) {
        const rows = await sequelize.query("SELECT name FROM schema_migrations WHERE name = ? LIMIT 1", {
        ...
        await sequelize.query("INSERT INTO schema_migrations (name, executed_at) VALUES (?, CURRENT_TIMESTAMP)", {
            replacements: [migration.name],
        });
    }
}
